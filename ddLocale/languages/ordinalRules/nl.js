/*
	Dutch (nl) ordinal rules
*/
function ddOrdinals(locale = 'nl', options = {}) {
	...
	function getOrdinal(v, o = false, s = false) {
		const _NS = [
			...
			{ val: 10**3, str: "duizend", short: "K" },
			{ val: 10**2, str: "honderd" },
			{ val: 90, str: "negentig" },
			{ val: 80, str: "tachtig" },
			{ val: 70, str: "zeventig" },
			{ val: 60, str: "zestig" },
			...
			{ val: 30, str: "dertig" },
			{ val: 20, str: "twintig" },
			{ val: 19, str: "negentien" },
			{ val: 18, str: "achtien" },
			{ val: 17, str: "zeventien" },
			{ val: 16, str: "zestien" },
			{ val: 15, str: "vijftien" },
			{ val: 14, str: "veertien" },
			{ val: 13, str: "dertien" },
			...
			{ val: 5, str: "vijf" },
			{ val: 4, str: "vier" },
			{ val: 3, str: "drie", ord: "der" },
			{ val: 2, str: "twee" },
			{ val: 1, str: "één", ord: "eer" },
			{ val: 0, str: "nul", ord: "" }
		];
		for (var n of _NS) {
			...
			if (v === n.val) return o === 'ordinal' && typeof n.ord !== 'undefined' ? `${n.ord}` : `${str}`;
				...
				if (!(v % n.val)) {
					let first = getOrdinal(v % 10);
					let between = first.slice(-1) === 'e' ? 'ën' : 'en';
					return `${first}${between}${getOrdinal(Math.floor(v / 10) * 10)}`;
				}
			}
		}
